  ...
  const handleCheckout = async () => {
    try {
      ...
      if (!result?.success && result?.cartCleared) {
        showToast.success(`Order has been placed successfully.`, ' Order Placed ');
  ...
  result?.order;
      ...
      try {
        const paymentData = {
          ...
          method: paymentMethod === 'card' ? 'card' : 'cash', // Use "cash" not "cod"
        ...
        // Payment API failed but order was created
        console.error('💳 Payment Error:', err);
        console.error('Response:', err.response?.data);
        
        // Still show success since order was created
        if (paymentMethod === 'card') {
          showToast.warning('Order created but payment processing failed. Please contact support.', 'Payment Issue');
        } else {
          // For cash, payment failure is less critical
          showToast.success('Order placed successfully! Pay on delivery.', 'Cash on Delivery');
        }
